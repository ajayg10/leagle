...
SAMPLE_POLICIES = [
    ...
    {
        "title": "Enterprise AI Deployment & Model Governance Policy",
        "text": """
        Purpose & Scope:
        This internal policy establishes operational standards for deploying Machine Learning, Generative AI, and LLM applications across production environments.

        1. Access & Credentials:
        - API keys for external foundation models (OpenAI, Anthropic, Gemini) must be stored in encrypted secret managers and never hardcoded in client-side repositories.
        - Direct database connections by autonomous AI agents are strictly restricted to read-only replicas.

        2. Data Privacy & Model Ingestion:
        - Sensitive Customer Personal Data (PII) must be masked or tokenized prior to ingestion into commercial LLM prompts.
        - Prompt histories and model response logs are retained in cold storage for 90 days for internal diagnostic review.

        3. Operational Monitoring:
        - Engineering teams must track API latency and token usage limits weekly.
        - Deployments facing customer-facing chatbots require quarterly human spot-checks.

        4. Current Exclusions & Gaps:
        - This policy does not currently enforce continuous automated bias auditing or algorithmic risk classification for high-risk employment systems.
        - Synthetic media generated by internal systems does not currently include cryptographic watermarking or automated machine-readable disclosures.
        """,
        "category": "artificial_intelligence",
        "department": "Engineering & Product",
        "owner": "Head of AI Architecture",
    },
]
